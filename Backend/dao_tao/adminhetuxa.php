...
if (!in_array('EDIT_NOIDUNG_HETUXA', $user_permissions)) {
...
if ($is_editing) {
    ...
    $stmt_edit = $conn->prepare("SELECT * FROM noidung_hetuxa WHERE id = ?"); // Đổi tên bảng
    if($stmt_edit){
        $stmt_edit->bind_param("i", $id); $stmt_edit->execute();
        $res_edit = $stmt_edit->get_result(); $edit_data = $res_edit->fetch_assoc();
        $stmt_edit->close();
         if (!$edit_data) { echo "<p style='color: red;'>Lỗi: Không tìm thấy nội dung Hệ Từ xa ID " . htmlspecialchars($id) . "</p>"; $is_editing = false; $id = null; }
    } else { echo "<p style='color: red;'>Lỗi DB: " . $conn->error . "</p>"; $is_editing = false; $id = null; }
}

global $page; $current_page_param = $page ?? 'daotao_tuxa';

// --- Xử lý POST ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_hetuxa'])) { // Thêm name cho nút submit
    $muc = $_POST['muc'] ?? '';
    ...
    if (!empty($post_id)) { // Cập nhật
        ...
        $query = "UPDATE noidung_hetuxa SET muc=?, tieude=?, noidung=?, hinhanh=? WHERE id=?"; // Đổi tên bảng
        $stmt_update = $conn->prepare($query);
        if($stmt_update){
            $stmt_update->bind_param("ssssi", $muc, $tieude, $noidung, $filename, $update_id);
            if($stmt_update->execute()){
                // Sửa chuyển hướng
                echo "<script>window.location.href = 'index.php?page=daotao_tuxa&status=updated';</script>"; exit();
            } else { echo "<p style='color:red;'>Lỗi cập nhật: ".$stmt_update->error."</p>"; }
            $stmt_update->close();
        } else { echo "<p style='color:red;'>Lỗi DB: ".$conn->error."</p>"; }
    } else { // Thêm mới
        $query = "INSERT INTO noidung_hetuxa (muc, tieude, noidung, hinhanh, ngaydang) VALUES (?, ?, ?, ?, NOW())"; // Đổi tên bảng
        $stmt_insert = $conn->prepare($query);
        if($stmt_insert){
             $stmt_insert->bind_param("ssss", $muc, $tieude, $noidung, $filename);
             if($stmt_insert->execute()){
                 // Sửa chuyển hướng
                 echo "<script>window.location.href = 'index.php?page=daotao_tuxa&status=added';</script>"; exit();
...
if (isset($_GET['delete'])) {
    ...
    $stmt_delete = $conn->prepare("DELETE FROM noidung_hetuxa WHERE id = ?"); // Đổi tên bảng
    if($stmt_delete){
        $stmt_delete->bind_param("i", $delete_id);
        if($stmt_delete->execute()){
             // ... (xóa file ảnh nếu cần) ...
             // Sửa chuyển hướng
             echo "<script>window.location.href = 'index.php?page=daotao_tuxa&status=deleted';</script>"; exit();
...
<!-- Sửa form action -->
<form action="index.php?page=daotao_tuxa<?php echo $is_editing ? '&id=' . $id : ''; ?>" method="POST" enctype="multipart/form-data">
    ...
    <button type="submit" name="submit_hetuxa" class="submit-button">
        <?php echo $is_editing ? 'Cập nhật' : 'Lưu nội dung'; ?>
    </button>

    <?php if ($is_editing): ?>
        <!-- Sửa link Huỷ sửa -->
        <a href="index.php?page=daotao_tuxa" class="button-link">Huỷ sửa</a>
    <?php endif; ?>
</form>
...
<table border="1" cellpadding="10" cellspacing="0" class="table">
    ...
    <?php
    // Lấy danh sách
    $res_list = $conn->query("SELECT id, muc, tieude, hinhanh FROM noidung_hetuxa ORDER BY id DESC"); // Đổi tên bảng
    ...
    ?>
        <tr>
            ...
            <td>
                <!-- Sửa link Sửa -->
                <a href="index.php?page=daotao_tuxa&id=<?php echo $r['id']; ?>" class='edit'>Sửa</a>
                <!-- Sửa link Xoá -->
                <a href="index.php?page=daotao_tuxa&delete=<?php echo $r['id']; ?>" class='delete' onclick="return confirm('Bạn chắc chắn muốn xoá?')">Xoá</a>
            </td>
        </tr>
    <?php
        endwhile;
