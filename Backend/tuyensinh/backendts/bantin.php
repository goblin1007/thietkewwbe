...
if (!in_array('EDIT_NOIDUNG_BANTIN', $user_permissions)) {
...
if ($is_editing) {
    ...
    $stmt_edit = $conn->prepare("SELECT * FROM noidung_bantin WHERE id = ?");
    if($stmt_edit){
        ...
        $stmt_edit->close();
         if (!$edit_data) {
             echo "<p style='color: red;'>Lỗi: Không tìm thấy Bản tin ID " . htmlspecialchars($id) . "</p>";
...
$current_page_param = $page ?? 'tuyensinh_bantin'; // Trang hiện tại

// Xử lý submit form
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_bantin'])) { // Thêm name vào nút submit
    $muc = $_POST['muc'] ?? '';
    ...
    if (!empty($post_id)) { // Cập nhật
        ...
        // Sử dụng prepared statement
        $query = "UPDATE noidung_bantin SET muc=?, tieude=?, noidung=?, hinhanh=? WHERE id=?";
        $stmt_update = $conn->prepare($query);
        if ($stmt_update) {
            $stmt_update->bind_param("ssssi", $muc, $tieude, $noidung, $filename, $update_id);
            if($stmt_update->execute()){
                // Sửa chuyển hướng
                echo "<script>window.location.href = 'index.php?page=tuyensinh_bantin&status=updated';</script>"; exit();
            } else { echo "<p style='color:red;'>Lỗi cập nhật: " . $stmt_update->error . "</p>"; }
            $stmt_update->close();
        } else { echo "<p style='color:red;'>Lỗi DB: " . $conn->error . "</p>"; }
    } else { // Thêm mới
         // Sử dụng prepared statement
        $query = "INSERT INTO noidung_bantin (muc, tieude, noidung, hinhanh, ngaydang) VALUES (?, ?, ?, ?, NOW())";
        $stmt_insert = $conn->prepare($query);
        if ($stmt_insert) {
            $stmt_insert->bind_param("ssss", $muc, $tieude, $noidung, $filename);
             if($stmt_insert->execute()){
                 // Sửa chuyển hướng
                echo "<script>window.location.href = 'index.php?page=tuyensinh_bantin&status=added';</script>"; exit();
...
if (isset($_GET['delete'])) {
    ...
    $stmt_delete = $conn->prepare("DELETE FROM noidung_bantin WHERE id = ?");
     if($stmt_delete){
        $stmt_delete->bind_param("i", $delete_id);
        if($stmt_delete->execute()){
            // ... (xóa file ảnh) ...
            // Sửa chuyển hướng
            echo "<script>window.location.href = 'index.php?page=tuyensinh_bantin&status=deleted';</script>"; exit();
...
if (isset($_GET['status'])) {
    ...
    $message = '';
    switch ($status) {
        case 'added': $message = 'Thêm bản tin thành công!'; break;
        case 'updated': $message = 'Cập nhật bản tin thành công!'; break;
        case 'deleted': $message = 'Xóa bản tin thành công!'; break;
    }
    if ($message) {
...
<!-- Sửa form action -->
<!-- Action trỏ về trang hiện tại (bantin) trong index.php -->
<form action="index.php?page=tuyensinh_bantin<?php echo $is_editing ? '&id=' . $id : ''; ?>" method="POST" enctype="multipart/form-data">
    ...
    if (!empty($edit_data['hinhanh']) && file_exists(__DIR__ . "/uploads/" . $edit_data['hinhanh'])) {
        // Đường dẫn tương đối từ index.php
    ...
    <button type="submit" name="submit_bantin" class="submit-button">
        <?php echo $is_editing ? 'Cập nhật' : 'Lưu nội dung'; ?>
    </button>

    <?php if ($is_editing): ?>
        <!-- Sửa link Huỷ sửa -->
        <a href="index.php?page=tuyensinh_bantin" class="button-link">Huỷ sửa</a>
    <?php endif; ?>
</form>
...
<table border="1" cellpadding="10" cellspacing="0" class="table">
    ...
    <?php
    // Lấy danh sách
    // Nên dùng prepared statement ở đây nếu có thể, nhưng tạm giữ query trực tiếp
    $res_list = $conn->query("SELECT id, muc, tieude, hinhanh FROM noidung_bantin ORDER BY id DESC");
    ...
    ?>
        <tr>
            ...
            <td>
                <!-- Sửa link Sửa -->
                <a href="index.php?page=tuyensinh_bantin&id=<?php echo $r['id']; ?>" class='edit'>Sửa</a>
                <!-- Sửa link Xoá -->
                <a href="index.php?page=tuyensinh_bantin&delete=<?php echo $r['id']; ?>" class='delete' onclick="return confirm('Bạn chắc chắn muốn xoá?')">Xoá</a>
            </td>
        </tr>
    <?php
        endwhile;
    else:
        echo '<tr><td colspan="5" style="text-align: center;">Chưa có bản tin nào.</td></tr>';
    endif;
