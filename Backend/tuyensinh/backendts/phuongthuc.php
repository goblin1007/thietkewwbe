...
if (!in_array('EDIT_NOIDUNG_PHUONGTHUC', $user_permissions)) {
...
if ($is_editing) {
    $stmt_edit = $conn->prepare("SELECT * FROM noidung_phuongthuc WHERE id = ?");
    if($stmt_edit){
        ...
        $stmt_edit->close();
         if (!$edit_data) {
             echo "<p style='color: red;'>Lỗi: Không tìm thấy ID " . htmlspecialchars($id) . "</p>";
...
$current_page_param = $page ?? 'tuyensinh_phuongthuc'; // Trang hiện tại

// Xử lý submit form
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_phuongthuc'])) { // Thêm name vào nút submit
    $muc = $_POST['muc'] ?? '';
    ...
    if (!empty($_FILES['hinhanh']['name'])) {
        $upload_dir_relative = "uploads/"; // Thư mục uploads trong backendts
        ...
        // Kiểm tra và upload (giữ nguyên logic)
        if (move_uploaded_file($_FILES['hinhanh']['tmp_name'], $target_file)) {
            $filename = $filename_new; // Update filename nếu thành công
            ...
            echo "<p style='color:red;'>⚠️ Có lỗi khi upload ảnh.</p>";
             // Giữ nguyên ảnh cũ nếu upload lỗi
    ...
    if (!empty($post_id)) { // Cập nhật
        ...
        $query = "UPDATE noidung_phuongthuc SET muc=?, tieude=?, noidung=?, hinhanh=? WHERE id=?";
        $stmt_update = $conn->prepare($query);
        if ($stmt_update) {
            $stmt_update->bind_param("ssssi", $muc, $tieude, $noidung, $filename, $update_id);
            if($stmt_update->execute()){
                echo "<script>window.location.href = 'index.php?page=tuyensinh_phuongthuc&status=updated';</script>"; exit();
            } else { echo "<p style='color:red;'>Lỗi cập nhật: " . $stmt_update->error . "</p>"; }
            $stmt_update->close();
        } else { echo "<p style='color:red;'>Lỗi DB: " . $conn->error . "</p>"; }
    } else { // Thêm mới
        $query = "INSERT INTO noidung_phuongthuc (muc, tieude, noidung, hinhanh, ngaydang) VALUES (?, ?, ?, ?, NOW())";
        $stmt_insert = $conn->prepare($query);
        if ($stmt_insert) {
            $stmt_insert->bind_param("ssss", $muc, $tieude, $noidung, $filename);
             if($stmt_insert->execute()){
                echo "<script>window.location.href = 'index.php?page=tuyensinh_phuongthuc&status=added';</script>"; exit();
...
if (isset($_GET['delete'])) {
    ...
    $stmt_delete = $conn->prepare("DELETE FROM noidung_phuongthuc WHERE id = ?");
     if($stmt_delete){
        $stmt_delete->bind_param("i", $delete_id);
        if($stmt_delete->execute()){
            // ... (xóa file ảnh) ...
            echo "<script>window.location.href = 'index.php?page=tuyensinh_phuongthuc&status=deleted';</script>"; exit();
...
// Hiển thị thông báo status (giữ nguyên)
if (isset($_GET['status'])) { /* ... code hiển thị thông báo ... */ }
?>
...
<!-- Sửa form action -->
<form action="index.php?page=tuyensinh_phuongthuc<?php echo $is_editing ? '&id=' . $id : ''; ?>" method="POST" enctype="multipart/form-data">
    ...
    <!-- ID của textarea này là "noidung", khớp với selector mặc định của TinyMCE nếu giữ nguyên -->
    <textarea name="noidung" id="noidung"><?php echo htmlspecialchars($form_data['noidung'] ?? ''); ?></textarea>

    <!-- Thêm name cho nút submit -->
    <button type="submit" name="submit_phuongthuc" class="submit-button">
        <?php echo $is_editing ? 'Cập nhật' : 'Lưu nội dung'; ?>
    </button>

    <?php if ($is_editing): ?>
        <!-- Sửa link Huỷ sửa -->
        <a href="index.php?page=tuyensinh_phuongthuc" class="button-link">Huỷ sửa</a>
    <?php endif; ?>
</form>
...
<table border="1" cellpadding="10" cellspacing="0" class="table">
    ...
    $res_list = $conn->query("SELECT id, muc, tieude, hinhanh FROM noidung_phuongthuc ORDER BY id DESC");
    ...
    ?>
        <tr>
            ...
            <td>
                <!-- Sửa link Sửa -->
                <a href="index.php?page=tuyensinh_phuongthuc&id=<?php echo $r['id']; ?>" class= 'edit'>Sửa</a>
                <!-- Sửa link Xoá -->
                <a href="index.php?page=tuyensinh_phuongthuc&delete=<?php echo $r['id']; ?>" class='delete' onclick="return confirm('Bạn chắc chắn muốn xoá?')">Xoá</a>
            </td>
        </tr>
    <?php
        endwhile;
    ...
    if ($res_list) $res_list->close(); // Giải phóng kết quả
    ?>
    </tbody>
</table>
